...
import {config as def_conf} from "./bili_ext.js";
import {config as loc_conf} from "http://127.0.0.1:9978/file/tvbox/%E9%85%8D%E7%BD%AE_Bili%E6%BA%90.js";

let siteKey = '';
let siteType = 0;
let config = {};
if(loc_conf.启用本地配置 != 1) {
	config = {
		cookie:def_conf.cookie,
        searchable:def_conf.searchable,
        homeSwitch:def_conf.homeSwitch,
		diyhome:def_conf.diyhome,
		homeName:def_conf.homeName,
        t_home:def_conf.t_home,
        t_history:def_conf.t_history,
        ratio:def_conf.ratio,
        type_name:def_conf.type_name,
        type_id:def_conf.type_id,
        filter:def_conf.filter
	}
} else {
	config = {
        ...
        t_history:loc_conf.显示历史记录页,
        ratio:loc_conf.视图比例,
        type_name:loc_conf.分类名,
        type_id:loc_conf.分类id,
        filter:loc_conf.筛选
	}
}
...
async function init(cfg) {
    ...
    //自定义分类页
    const ext_tname = btname.split('#');
    const ext_tid = btid.split('#');
	...
	for(let i = 0; i < cnt; i++) {
        ...
        newarr.push(c);
        //d[ext_tid[i]]=jsonData;
	};
    
    /*for (const kk of ext_tname) {
        const c = {
            type_name: kk,
            type_id: kk,
            land: 1,
...
async function homeVod() {
    if(config.homeSwitch==1){
        try {
            const url1 = 'https://api.bilibili.com/x/web-interface/index/top/rcmd?ps=14&fresh_idx=1&fresh_idx_1h=1';
            const url2 = 'https://api.bilibili.com/x/web-interface/search/type?search_type=video&keyword=' + config.homeName;
            const list = [];       
            if(config.diyhome!=1){
                const response = await request(url1, getHeaders());
            ...
            return JSON.stringify(result);
                ...
                const response = await request(url2, getHeaders());
                const responseData = JSON.parse(response);
                const vods = responseData.data.result;    

                for (const it of vods) {
                    const vod = {};
                    let imageUrl = it.pic;
                    if (imageUrl.startsWith('//')) {
                        imageUrl = 'https:' + imageUrl;
                    ...
                    vod.vod_id = it.bvid;
                    vod.vod_name = removeTags(it.title);
                    vod.vod_pic = imageUrl;
                    vod.vod_remarks = cd;
                    vod.style = {
                        type: 'rect',
            ...
            //const responseData = JSON.parse(await request(url, getHeaders()));
...
async function detail(ids) {
    try {
        ...
        const video = {
            ...
            vod_remarks: cd,
            vod_actor: '',
            vod_director: detailData.owner.name,
...
//1111111111111111111111111111111
        ...
        for (let j = 0; j < jSONArray.length; j++) {
            ...
            const part=jSONObject6.part.replace('#', '﹟').replace('$', '﹩');
            const playUrl = part + '$' + aid + '+' + cid + '+' + qualitylist.join(':') + '+' + descriptionList.join(':');
            playList.push(playUrl);
        }

        treeMap['B站'] = playList.join('#');
