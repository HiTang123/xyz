...
function print(any) {
	any = any || "";
	if(typeof any == "object" && Object.keys(any)
		.length > 0) {
		try {
	...
	} else if(typeof any == "object" && Object.keys(any)
		.length < 1) {
...
function isMedia(file) {
	return /\.(dff|dsf|mp3|aac|wav|wma|cda|flac|m4a|mid|mka|mp2|mpa|mpc|ape|ofr|ogg|ra|wv|tta|ac3|dts|tak|webm|wmv|mpeg|mov|ram|swf|mp4|avi|rm|rmvb|flv|mpg|mkv|m3u8|ts|3gp|asf|vob|m2ts)$/.test(file.toLowerCase());
}
...
function init(ext) {
	...
	print(drives);
	searchDriver = (drives.find(x => x.search) || {})
		.name || "";
	...
	drives.forEach(item => {
		...
		__drives[item.name] = {
			...
			getPic(data) {
				...
				return pic || (this.isFolder(data) ? "https://ghproxy.com/raw.githubusercontent.com/HiTang123/xyz/master/wallpaper/icon/文件夹1.png" : "https://ghproxy.com/raw.githubusercontent.com/HiTang123/xyz/master/wallpaper/icon/film2.png")
			},
...
function home(filter) {
	let classes = Object.keys(__drives)
		.map(key => ({
...
function category(tid, pg, filter, extend) {
	...
	if(fl.order) {
		let key = fl.order.split("_")
			.slice(0, -1)
			.join("_");
		let order = fl.order.split("_")
			.slice(-1)[0];
...
function getAll(otid, tid, drives, path) {
	try {
		...
		let vod = {
			...
			vod_name: vod_name,
			type_name: "文件夹",
			vod_pic: "https://ghproxy.com/raw.githubusercontent.com/HiTang123/xyz/master/wallpaper/icon/radio1.jpg",
			vod_content: tid,
			vod_tag: "folder",
		...
		let list = [{
			...
			vod_id: "no_data",
			vod_remarks: "不要点,会崩的",
			vod_pic: "https://ghproxy.com/raw.githubusercontent.com/hjdhnx/dr_py/main/404.jpg",
			vod_actor: e.message,
			vod_director: tid,
			vod_content: otid
		}];
...
function play(flag, id, flags) {
	...
	let vod = {
		parse: 0,
		playUrl: "",
		url: drives.getFile(urls[0])
			.raw_url
	};
	if(urls.length >= 2) {
		const path = urls[0].substring(0, urls[0].lastIndexOf("/") + 1);
		vod.subt = drives.getFile(path + urls[1])
			.raw_url1
	}
	print("----play----");
	print(vod);
	...
	if(!searchDriver || !wd) {
		...
		lists.forEach(it => {
			...
			vods.push({
				...
				vod_pic: "https://ghproxy.com/raw.githubusercontent.com/HiTang123/xyz/master/wallpaper/icon/fangying.png",
